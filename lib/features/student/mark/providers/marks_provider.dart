import 'package:ac_students/features/student/mark/model/mark.dart';
...
class MarksProvider extends ChangeNotifier {
  ...
  Future getMarks({required int userDep}) async {
    if (marks.isEmpty) {
      ...
      marks = allMarks
          .where((element) =>
              userDep == 1 ? element.subjectId! < 50 : element.subjectId! > 50)
          .toList();
      notifyListeners();
    } else {}
  }
  ...
  void deleteMarks(BuildContext context, String id) async {
    final marks = ParseObject('Marks')..set('objectId', id);
          ...
          showDialog(
            ...
            builder: (BuildContext context) {
              return AlertDialog(
                ...
                content: Text(
                    'the $fileName file has been downloaded to /Downloads/AppliedCollege'),
                actions: <Widget>[
                  TextButton(
                    child: const Text("OK"),
                    onPressed: () {
